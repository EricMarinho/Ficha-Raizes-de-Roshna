...
class Ficha {
    constructor(id, db) {
        this.id = id;
        this.db = db;
        this.dados = localStorage.getItem(`${id}_dados`) ? JSON.parse(localStorage.getItem(`${id}_dados`)) : {
            nomeJogador: '',
            nomePersonagem: '',
            idade: '',
            profissao: '',
        ...
        this.vida = localStorage.getItem(`${id}_vida`) ? JSON.parse(localStorage.getItem(`${id}_vida`)) : {
            min: 0,
            max: 0,
            temporaria: 0
        };
        this.normal = localStorage.getItem(`${id}_normal`) ? JSON.parse(localStorage.getItem(`${id}_normal`)) : false;
        this.ferido = localStorage.getItem(`${id}_ferido`) ? JSON.parse(localStorage.getItem(`${id}_ferido`)) : false;
        this.morrendo = localStorage.getItem(`${id}_morrendo`) ? JSON.parse(localStorage.getItem(`${id}_morrendo`)) : false;
        this.morto = localStorage.getItem(`${id}_morto`) ? JSON.parse(localStorage.getItem(`${id}_morto`)) : false;
        this.condicao = localStorage.getItem(`${id}_condicao`) ? JSON.parse(localStorage.getItem(`${id}_condicao`)) : '';
        this.armorClass = localStorage.getItem(`${id}_armorClass`) ? JSON.parse(localStorage.getItem(`${id}_armorClass`)) : '0';
        this.movimento = localStorage.getItem(`${id}_movimento`) ? JSON.parse(localStorage.getItem(`${id}_movimento`)) : '0';
        this.spellsSlots = localStorage.getItem(`${id}_spellsSlots`) ? JSON.parse(localStorage.getItem(`${id}_spellsSlots`)) : '0';
        this.profBonus = localStorage.getItem(`${id}_profBonus`) ? JSON.parse(localStorage.getItem(`${id}_profBonus`)) : '0';
        this.hitDiceValue = localStorage.getItem(`${id}_hitDiceValue`) ? JSON.parse(localStorage.getItem(`${id}_hitDiceValue`)) : '0';
        this.hitDiceQuantity = localStorage.getItem(`${id}_hitDiceQuantity`) ? JSON.parse(localStorage.getItem(`${id}_hitDiceQuantity`)) : '0';
        this.atributos = localStorage.getItem(`${id}_atributos`) ? JSON.parse(localStorage.getItem(`${id}_atributos`)) : atributos.map(x => ({ nome: x, valor: 0 }));
        this.combate = localStorage.getItem(`${id}_combate`) ? JSON.parse(localStorage.getItem(`${id}_combate`)) : [];
        this.spellsSlotsArray = localStorage.getItem(`${id}_spellsSlotsArray`) ? JSON.parse(localStorage.getItem(`${id}_spellsSlotsArray`)) : nivel.map((x, index) => ({
            nivel: x,
            quantidade: '0',
            magias: []
        }));
        this.genericRoll = localStorage.getItem(`${id}_genericRoll`) ? JSON.parse(localStorage.getItem(`${id}_genericRoll`)) : [];
        this.habilidades = localStorage.getItem(`${id}_habilidades`) ? JSON.parse(localStorage.getItem(`${id}_habilidades`)) : {
        ...
        this.pets = localStorage.getItem(`${id}_pets`) ? JSON.parse(localStorage.getItem(`${id}_pets`)) : [];
        this.inventario = localStorage.getItem(`${id}_inventario`) ? JSON.parse(localStorage.getItem(`${id}_inventario`)) : [];
        this.notas = localStorage.getItem(`${id}_notas`) ? JSON.parse(localStorage.getItem(`${id}_notas`)) : '';
        this.petSelecionado = localStorage.getItem(`${id}_petSelecionado`) ? JSON.parse(localStorage.getItem(`${id}_petSelecionado`)) : null;
        if (this.pets[0]) {
    ...
    imprimirAtributos() {
        ...
        this.atributos.forEach((x, index) => {
            const atributoInput = document.querySelector(`#atributos${index}`);
    ...
    imprimirCombate() {
        ...
        elementoPai.innerHTML = newHTML;
        this.combate.forEach((x, index) => {
            const nomeInput = document.querySelector(`#nomeCombate-${index}`);
            nomeInput.value = x.nome || '';
            const dadosInput = document.querySelector(`#dadosCombate-${index}`);
            dadosInput.value = x.dados || '';
            const rangeInput = document.querySelector(`#rangeCombate-${index}`);
    ...
    imprimirGenericRoll() {
        ...
        elementoPai.innerHTML = newHTML;
        this.genericRoll.forEach((x, index) => {
            const nomeInput = document.querySelector(`#nomegenericRoll-${index}`);
            nomeInput.value = x.nome || '';
            const dadosInput = document.querySelector(`#dadosgenericRoll-${index}`);
            dadosInput.value = x.dados || '';
        });
    }
    imprimirProficiencias(seletorTabela, item, idName, nomePet = null) {
        const elementoPai = document.querySelector(`.container${seletorTabela}`);
        let newHTML = `<table>
                <tr><td colspan="7">Saving Throws</td></tr>`;
        function Template(x, index, property) {
            newHTML += /* html */ ` <tr>
        ...
        propriedades.forEach(y => {
            item[y].forEach((x, index) => {
                const modInput = document.querySelector(`#${idName}mod${y}-${index}`);
                ...
                if (y === 'ataque' || y === 'combate') {
                    const nomeInput = document.querySelector(`#${idName}nome${y}-${index}`);
                    nomeInput.addEventListener("keyup", (event) => {
                        ficha.salvarDadosTabela(event, item[y][index], 'nome', idName);
                    });
                    nomeInput.value = x.nome || '';
                    const closeButton = document.querySelector(`#${idName}${y}Close-${index}`);
                    closeButton.addEventListener("click", (event) => {
                        ficha.apagarElementoArrayTabela(item[y], index, idName);
                    });
                }
                const dadoDesvantagemButton = document.querySelector(`#${idName}${y}buttonDesvantagemContainer-${index}`);
                dadoDesvantagemButton.addEventListener("click", (event) => {
                    ficha.gerarDado(x, 'desvantagem', 2, x.nome, nomePet);
                });
                const dadoButton = document.querySelector(`#${idName}${y}buttonContainer-${index}`);
                dadoButton.addEventListener("click", (event) => {
                    ficha.gerarDado(x, 'normal', 1, x.nome, nomePet);
                });
                const dadoVantagemButton = document.querySelector(`#${idName}${y}buttonVantagemContainer-${index}`);
        ...
        if (nomePet) {
            const inputProficienciaPet = document.querySelector('.inputProficienciaPet');
            inputProficienciaPet.value = item.proficienciaBonus || '';
            const inputVidaPet = document.querySelector('.inputVidaPet');
            inputVidaPet.value = item.vida || '';
            const inputAcPet = document.querySelector('.inputAcPet');
            inputAcPet.value = item.ac || '';
            const addButton = document.querySelector(`#${idName}combateAdd`);
            addButton.addEventListener("click", (event) => {
                ficha.adicionarElementoArrayTabela(item.combate);
            });
            item.combate.forEach((x, index) => {
                const nomeInput = document.querySelector(`#${idName}nomecombate-${index}`);
                nomeInput.addEventListener("keyup", (event) => {
                    ficha.salvarDadosTabela(event, item.combate[index], 'nome', idName);
                });
                nomeInput.value = x.nome || '';
                const closeButton = document.querySelector(`#${idName}combateClose-${index}`);
                closeButton.addEventListener("click", (event) => {
                    ficha.apagarElementoArrayTabela(item.combate, index, idName);
                });
                const dadoInput = document.querySelector(`#${idName}dadoscombate-${index}`);
                ...
                const rangeInput = document.querySelector(`#${idName}rangecombate-${index}`);
                rangeInput.addEventListener("keyup", (event) => {
                    ficha.salvarDadosTabela(event, item.combate[index], 'range', idName);
                });
                rangeInput.value = x.range || '';
                const dadoCombateButton = document.querySelector(`#${idName}combatebuttonContainer-${index}`);
                dadoCombateButton.addEventListener("click", (event) => {
                    ficha.gerarDadoCombatePets(index, nomePet);
                });
            });
        }
        const addButton = document.querySelector(`#${idName}ataqueAdd`);
        addButton.addEventListener("click", (event) => {
    ...
    imprimirListaMagias() {
        ...
        elementoPai.innerHTML = newHTML;
        this.spellsSlotsArray.forEach((x, index) => {
            const nivelInput = document.querySelector(`#quantidadespellsSlotsArray-${index}`);
            nivelInput.value = x.quantidade || '';
            x.magias.forEach((y, idx) => {
                const nomeInput = document.querySelector(`#magiasNome-${index}-${idx}`);
                nomeInput.value = y.nome || '';
                const rngInput = document.querySelector(`#magiasRng-${index}-${idx}`);
                rngInput.value = y.range || '';
                const lvInput = document.querySelector(`#magiasLv-${index}-${idx}`);
                lvInput.value = y.nivel || '';
                const castInput = document.querySelector(`#magiasCast-${index}-${idx}`);
                castInput.value = y.castingTime || '';
                const tempoInput = document.querySelector(`#magiasTempo-${index}-${idx}`);
                tempoInput.value = y.duracao || '';
                const botaoExcluirSpell = document.querySelector(`#botaoExcluirSpell-${index}-${idx}`);
                botaoExcluirSpell.addEventListener("click", (event) => {
                    ficha.apagarElementoArrayTabela(x.magias, idx, 'spellsSlotsArray');
                });
            });
            const botaoAdicionarSpell = document.querySelector(`#botaoAdicionarSpell-${index}`);
    ...
    imprimirInventario() {
        ...
        elementoPai.innerHTML = newHTML;
        this.inventario.forEach((x, index) => {
            const nomeInput = document.querySelector(`#nomeInventario-${index}`);
            nomeInput.value = x.nome || '';
            const pesoInput = document.querySelector(`#pesoInventario-${index}`);
    ...
    imprimirFicha() {
        ...
        const Input = document.querySelector(`#vidaInput`);
        Input.value = this.vida.min + '/' + this.vida.max;
        const Input2 = document.querySelector(`#vidaTempInput`);
        ...
        camposTexto.forEach(x => {
            const elemento = document.querySelector(`#${x[0]}`);
            elemento.value = x[1];
        });
        ...
        checkboxStatus.forEach((x) => {
            const Input = document.querySelector(`#status${x}`);
    ...
    imprimirListaPets() {
        ...
        elementoPai.innerHTML = newHTML;
        elementoPai.value = this.petSelecionado;
    }
    adicionarPet() {
        const elementoPai = document.querySelector('.petsSelect');
    ...
    adicionarElementoArray(property) {
        this[property].push({});
        this.imprimirFicha();
    }
    adicionarElementoArrayTabela(item) {
        item.push({});
        this.imprimirFicha();
    }
    apagarElementoArrayTabela(item, index, title) {
        item.splice(index, 1);
        localStorage.setItem(`${this.id}_${title}`, JSON.stringify(this[title]));
        this.imprimirFicha();
    }
    apagarElementoArray(index, property) {
        this[property].splice(index, 1);
        localStorage.setItem(`${this.id}_${property}`, JSON.stringify(this[property]));
        this.imprimirFicha();
    }
    salvarDadosMagias(e, index, idx, property) {
    ...
    deathSave(e, isDeathSave) {
        const deathSaveContainer = document.querySelector("#deathSaveContainer");
        if (!isDeathSave) {
    ...
    gerarDado(item, rollType, times, title, petName = null) {
        let newHTML = '';
        const dadosGirados = [];
        const resultados = [];
    ...
    gerarDadoCombate(index, property = 'combate', title = 'Combate', petName = null) {
        let result = [];
        result.push({
            value: 0
        });
    ...
    gerarDadoCombatePets(index, petName) {
        let result = [];
        result.push({
            value: 0
        });
    ...
    textoModal(dado, resultado, dadoTexto = "Dados") {
